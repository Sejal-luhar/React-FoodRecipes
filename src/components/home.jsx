...
function HomePage() {
  ...
  return (
    ...
    <NavBar onSearch={handleSearch} />
      {/* Hero Section */}
      <div className="relative h-[90vh] bg-cover bg-center" style={{ backgroundImage: "url('/public/dinner.jpeg')" }}>
        ...
        <div className="relative flex flex-col items-center justify-center h-full text-center text-white px-6">
          ...
          <Link to="/recipes" className="px-8 py-4 opacity-90 bg-gradient-to-br from-red-500 via-orange-500 to-orange-300 text-lg rounded-full hover:bg-orange-600 transition shadow-white">
            Browse Recipes
          </Link>
        </div>
      </div>
      ...
      <section className="py-12 bg-gray-100">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 px-6 md:px-16">
          {categories.map((category, index) => (
            <div key={index} className="relative group rounded-full shadow-md shadow-black overflow-hidden">
              <div className="w-full h-44 bg-cover  bg-center rounded-lg shadow-md" style={{ backgroundImage: `url(${category.image})` }}></div>
            ...
            {filteredRecipes.map((recipe) => (
              ...
              >
                ...
                <div className="py-2 text-center">
                  <h3 className=" font-semibold text-gray-700">{recipe.strMeal}</h3>
                  <Link
